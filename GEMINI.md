...
**Trials are the layer-1 challenges, and Royal Jelly buys the door.** **The Trials** is the thirteenth adaptation at 8 jelly, so the tree that used to end with "there is nothing left here to buy" now ends by opening a tab. Entering one founds a colony under conditions that should kill it. Six are listed; only **Drought** runs, and the other five say plainly that their shape is still being decided rather than pretending to be locked content.

**The lineage's automation comes with you; its strength does not.** `prestige_1`–`8` and the ×25 big-forager bonus all return neutral inside a trial, `prestige_9`–`12` do not. Tedium is not difficulty, and six trials of manual laying is not a challenge — it is the same challenge six times with the fun removed. `prestige.js` reads `game.challenge` directly for that check rather than importing `challenges.js`, which imports it back.

**The target never moves and the debuff does.** Every level of every trial asks for the same 600 ants; difficulty is entirely in the debuff, and the reward a completion pays is what lets you meet the next one. Drought runs at `0.4 × 0.6^level` food, and every level cleared anywhere pays `1.3^levels` to every colony, inside trials as well as outside. **The reward has to apply inside or there is no race** — each level would be strictly harder with nothing to meet it and the ladder would stall at two. Net difficulty runs 0.400, 0.312, 0.243, 0.190, 0.148, 0.115 across the first six levels.
...
**Not built.** Prestige layers beyond the first, and five of the six trials. A competent player finishes the Royal Lineage in about five hours and then has Drought to climb, so that is the current edge of the game.
