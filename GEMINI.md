...
**Trials are the layer-1 challenges, and Royal Jelly buys the door.** **The Trials** is the thirteenth adaptation at 8 jelly, so the tree that used to end with "there is nothing left here to buy" now ends by opening a tab. Entering one founds a colony under conditions that should kill it. Six are listed; only **Drought** runs, and the other five say plainly that their shape is still being decided rather than pretending to be locked content. Every card states its debuff, its target and both its rewards as numbers, and the hover lists what a trial takes, what comes with you, what clears it and what clearing pays — players could not tell that the lineage stays behind at all, and asked.
